				...
				<div className="fade-in animate-in space-y-10 duration-500">
						...
						<div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
							<fieldset className="fieldset min-w-0">
								...
								<p className="label text-[10px] opacity-60">
									Base path relative to mount point (usually /).
								</p>
							</fieldset>
							...
							<fieldset className="fieldset min-w-0">
								...
								/>
								<p className="label text-[10px] opacity-60">
									The URL ARR instances will use to communicate.
								</p>
							</fieldset>
						</div>
					</section>
					...
					<section className="space-y-6">
						...
						<div className="grid grid-cols-1 gap-6 rounded-2xl border border-base-300 bg-base-200/30 p-6 lg:grid-cols-2">
							<div className="space-y-4">
								...
								<fieldset className="fieldset min-w-0">
									<legend className="fieldset-legend font-semibold">
										API Key{" "}
										{config.sabnzbd.fallback_api_key_set && (
											<span className="badge badge-success badge-xs ml-1 origin-left scale-75 uppercase">
												Set
											</span>
										)}
									</legend>
							...
							<div className="flex flex-col justify-center">
								<div
									className={`alert ${formData.fallback_host ? "alert-info" : "bg-base-100/50"} py-3 text-xs leading-relaxed`}
								>
									<AlertTriangle className="h-4 w-4 shrink-0" />
									<p>
										Failed imports can be automatically forwarded to a real SABnzbd instance for
										repair or alternate handling.
									</p>
								</div>
							</div>
						</div>
					...
					<section className="space-y-6">
						<div className="flex items-center justify-between">
							...
							<button
								type="button"
								className="btn btn-xs btn-primary btn-outline ml-4 px-4"
						...
						{showAddCategory && (
							<div className="card fade-in animate-in border-2 border-primary/30 bg-base-100 shadow-md duration-300">
								<div className="card-body p-4">
									<h5 className="mb-4 font-bold text-primary text-xs uppercase">
										Add New Category
									</h5>
									<div className="grid grid-cols-1 items-end gap-4 sm:grid-cols-2 lg:grid-cols-4">
										<fieldset className="fieldset min-w-0">
											<legend className="fieldset-legend font-bold text-[10px] opacity-50">
												NAME
											</legend>
											<input
												type="text"
										...
										<fieldset className="fieldset min-w-0">
											...
											<input
												type="text"
												className="input input-xs w-full bg-base-200 font-mono"
										...
										<div className="grid grid-cols-2 gap-4">
											...
											<fieldset className="fieldset min-w-0">
												...
												<select
													className="select select-xs w-full bg-base-200"
							...
							{formData.categories
								...
								.map((category, index) => {
									...
									return (
										<div
											key={index}
											className={`card ${isDefault ? "border-primary/20 bg-primary/5" : "border-base-300 bg-base-200/50"} border shadow-sm`}
										>
											<div className="card-body p-4">
												<div className="grid grid-cols-1 items-end gap-4 sm:grid-cols-2 lg:grid-cols-4">
													<fieldset className="fieldset min-w-0">
														...
														<input
															type="text"
															className="input input-xs w-full bg-base-100 font-bold"
															value={category.name}
															disabled={isReadOnly || isDefault}
															onChange={(e) =>
													...
													<fieldset className="fieldset min-w-0">
														...
														<input
															type="text"
															className="input input-xs w-full bg-base-100 font-mono"
															value={category.dir}
															disabled={isReadOnly}
															placeholder="Optional"
													...
													<div className="grid grid-cols-2 gap-4">
														...
														<fieldset className="fieldset min-w-0">
															...
															<select
																className="select select-xs w-full bg-base-100"
													...
													<div className="flex justify-end">
														...
														{isDefault && (
															<span className="badge badge-primary badge-outline badge-xs px-2 font-bold uppercase">
																System
															</span>
														)}
													</div>
					...
					{!isReadOnly && (
						<div className="flex flex-col gap-6 border-base-200 border-t pt-6">
							<div className="flex flex-wrap justify-end gap-3">
								...
								>
									<CheckCircle className="h-4 w-4" /> Test Connectivity
								</button>
								<button
									type="button"
									className="btn btn-outline btn-info btn-sm px-6"
									onClick={handleRegisterDownloadClient}
									disabled={registerDownloadClient.isPending}
								>
									<Download className="h-4 w-4" /> Auto-Setup ARR
								</button>
								<button
									type="button"
									...
									disabled={!canSave}
								>
									<Save className="h-4 w-4" /> Save SABnzbd Configuration
								</button>
							</div>

							{testResults && (
								<div className="fade-in animate-in rounded-xl border border-base-300 bg-base-200/50 p-4 duration-300">
									<span className="mb-3 block font-bold text-[10px] uppercase opacity-50">
									...
									<div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3">
										{Object.entries(testResults).map(([instance, result]) => (
											<div
												key={instance}
												className="flex items-center justify-between rounded-lg border border-base-200 bg-base-100 p-2 px-3"
											>
												<span className="mr-2 truncate font-semibold text-xs">{instance}</span>
												<span
													className={`rounded-full px-2 py-0.5 font-mono text-[10px] ${result === "OK" ? "bg-success/10 text-success" : "bg-error/10 text-error"}`}
												>
													{result}
												</span>
											</div>
				...
				<div className="alert border-info/20 bg-info/5 py-6 shadow-sm">
					...
					<div>
						...
						<p className="mt-1 text-sm opacity-80">
							Enable this to allow Radarr/Sonarr to communicate with AltMount as if it were a local
							SABnzbd instance.
						</p>
					</div>
				</div>
