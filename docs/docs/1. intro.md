...
- **Symlink Support**: Optional category-based symlinks for cleaner filesystem organization
...
5. **(Optional) Enable Symlinks**: For advanced users who need filesystem-based paths:

   - Toggle **Enable Symlinks**
   - Set **Symlink Directory** to an absolute path (e.g., `/symlinks`)
   - This creates category-based symbolic links for imported files
   - See [Symlink Configuration](Configuration/integration#symlink-configuration-advanced) for detailed setup
