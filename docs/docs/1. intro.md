...
### 2. Configure Web Interface Access

**Important**: Before accessing the web interface, ensure the `COOKIE_DOMAIN` environment variable is set correctly:

- If accessing via `localhost` or `127.0.0.1`: Set `COOKIE_DOMAIN=localhost`
- If accessing via a specific IP (e.g., `192.168.1.100`): Set `COOKIE_DOMAIN=192.168.1.100`
- If accessing via a domain (e.g., `altmount.example.com`): Set `COOKIE_DOMAIN=altmount.example.com`

The `COOKIE_DOMAIN` must match exactly the domain or IP address you use to access the web interface, otherwise login will fail due to cookie domain mismatch.

### 3. Configure Usenet Providers
...
### 6. Enable SABnzbd Compatibility
...
### 7. Configure Your ARR Applications
...
### 8. Add ARR Integration (Optional)
