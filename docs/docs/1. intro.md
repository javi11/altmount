...
5.  **Choose Import Strategy**: Select how imported files should be made available to external applications:
    ...
    - **STRM**: Generates STRM streaming files that point to the WebDAV content. Works with Emby, Jellyfin, and other media servers that support STRM files. - Toggle **Enable STRM** - Set **STRM Directory** to an absolute path (e.g., `/strm`) - STRM files will be organized by category in the STRM directory + See [Import Strategy Configuration](Configuration/integration#import-strategy-configuration) for detailed setup and advanced options. + 6. **(Optional) Enable Fallback Host**: For advanced users who need to send failed imports to an external SABnzbd instance: - Toggle **Enable Fallback Host** - Set **Fallback Host** to the URL of the external SABnzbd instance - Set **Fallback API Key** to the API key of the external SABnzbd instance - This will send failed imports to the external SABnzbd instance - See [Fallback Host Configuration](Configuration/providers#provider-priority-and-backup-strategy) for detailed setup 7. **Save** the configuration
...
- - Set to the path where your media library files are stored (e.g., `/mnt/library`)
...
4.  **(Optional) Enable Cleanup Orphaned Metadata Files**: - Toggle this option to automatically delete metadata files that no longer have corresponding files in your library
