...
1. **Trigger** — a repair job is queued when: a stream hits a confirmed-missing article, the health checker classifies a file as degraded, or you `POST /api/par2repair`.
