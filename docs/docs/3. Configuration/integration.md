...
![ARR integration settings showing Radarr/Sonarr instance configuration](../../static/img/arr_config.png)
...
- `mount_path` -- the path where the WebDAV/FUSE content is mounted, as seen by the ARR apps

> **Note:** Both AltMount and your ARR apps must be able to reach `import_dir` and `mount_path`. In Docker, this typically means shared volumes.

### Example Docker Volume Setup
...
With this layout, both containers see the same paths, so `import_dir: /data/imports` and `mount_path: /data/media` will work correctly.
...
| `queue_cleanup_allowlist` | Additional error messages to treat as safe for cleanup |
