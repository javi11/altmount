...
-- Create temporary table with new constraint that includes 'pending'
...
-- Create temporary table with original constraint (without 'pending')
CREATE TABLE file_health_temp (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    file_path TEXT NOT NULL UNIQUE,
...
-- Convert any 'pending' status to 'partial' when rolling back
INSERT INTO file_health_temp 
SELECT id, file_path, 
       CASE WHEN status = 'pending' THEN 'partial' ELSE status END,
