...
SET virtual_path = ltrim(replace(virtual_path, chr(92), '/'), '/')
WHERE virtual_path <> ltrim(replace(virtual_path, chr(92), '/'), '/');

DROP TABLE IF EXISTS file_health_path_normalized;

CREATE TEMP TABLE file_health_path_normalized AS
WITH normalized AS (
    SELECT h.*, ltrim(replace(h.file_path, chr(92), '/'), '/') AS canonical_path
    FROM file_health h
), paths AS (
...
SELECT
    ...
    (SELECT BOOL_OR(COALESCE(h.is_masked, FALSE)) FROM normalized h
     WHERE h.canonical_path = p.canonical_path) AS is_masked,
    (SELECT h.metadata FROM normalized h
     WHERE h.canonical_path = p.canonical_path AND h.metadata IS NOT NULL
