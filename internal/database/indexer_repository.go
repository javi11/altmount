...
func (r *Repository) LogIndexerImport(ctx context.Context, indexer string, status string, errMsg string, downloadID *string) error {
	...
	query := `
		...
		VALUES (?, ?, ?, ?, datetime('now'))
	`
	_, err := r.db.ExecContext(ctx, query, indexer, status, errDetails, downloadID)
	return err
}
...
// DeleteIndexerStats deletes all records for a specific indexer.
func (r *Repository) DeleteIndexerStats(ctx context.Context, indexer string) error {
	query := `DELETE FROM indexer_import_stats WHERE indexer = ?`
	_, err := r.db.ExecContext(ctx, query, indexer)
	return err
}
...
// DeleteIndexerStats deletes all records for a specific indexer.
func (r *QueueRepository) DeleteIndexerStats(ctx context.Context, indexer string) error {
	query := `DELETE FROM indexer_import_stats WHERE indexer = ?`
	_, err := r.db.ExecContext(ctx, query, indexer)
	return err
}
