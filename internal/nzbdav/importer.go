...
const (
	StatusIdle      ImportStatus = "idle"
	StatusRunning   ImportStatus = "running"
	StatusCanceling ImportStatus = "canceling"
)

// ImportInfo holds information about the current import operation
...
// Importer handles importing from NZBDav databases
...
// NewImporter creates a new NZBDav importer
func NewImporter(batchAdder BatchQueueAdder) *Importer {
	return &Importer{
		batchAdder: batchAdder,
		log:        slog.Default().With("component", "nzbdav-importer"),
		info:       ImportInfo{Status: StatusIdle},
	}
}

// Start starts an asynchronous import from an NZBDav database
func (n *Importer) Start(dbPath string, rootFolder string, nzbDir string, cleanupFile bool) error {
	n.mu.Lock()
	defer n.mu.Unlock()

	if n.info.Status != StatusIdle {
...
func (n *Importer) GetStatus() ImportInfo {
	n.mu.RLock()
	defer n.mu.RUnlock()
	return n.info
}
...
func (n *Importer) Cancel() error {
	...
	if n.info.Status == StatusCanceling {
		return fmt.Errorf("import is already being canceled")
	}

	n.info.Status = StatusCanceling
	if n.cancelFunc != nil {
		n.cancelFunc()
	}
...
func (n *Importer) performImport(ctx context.Context, dbPath string, rootFolder string, nzbDir string, cleanupFile bool) {
	// Parse Database
...
func (n *Importer) processBatch(ctx context.Context, batchChan <-chan *database.ImportQueueItem) {
...
func (n *Importer) createNzbFileAndPrepareItem(ctx context.Context, res *ParsedNzb, rootFolder, nzbTempDir string) (*database.ImportQueueItem, error) {
