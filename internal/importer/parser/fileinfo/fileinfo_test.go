...
func TestSelectBestFilenameQuotedSubject(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "single-quoted RAR subject strips surrounding quotes",
			par2Filename:    "",
			subjectFilename: "'Some.Show.S03.WEBRip.EAC3.2.0.1080p.x265-GRP.part001.rar'",
			headerFilename:  "",
			subjectHeader:   "",
			want:            "Some.Show.S03.WEBRip.EAC3.2.0.1080p.x265-GRP.part001.rar",
		},
		{
			name:            "double-quoted RAR subject strips surrounding quotes",
			par2Filename:    "",
			subjectFilename: `"X.part01.rar"`,
			headerFilename:  "",
			subjectHeader:   "",
			want:            "X.part01.rar",
		},
		{
			name:            "single-quoted obfuscated hash RAR subject strips surrounding quotes",
			par2Filename:    "",
			subjectFilename: "'a1b2c3d4.part01.rar'",
			headerFilename:  "",
			subjectHeader:   "",
			want:            "a1b2c3d4.part01.rar",
		},
		{
			name:            "single-quoted subject with no RAR extension strips surrounding quotes",
			par2Filename:    "",
			subjectFilename: "'Some.Show.S03'",
			headerFilename:  "",
			subjectHeader:   "",
			want:            "Some.Show.S03",
		},
		{
			name:            "single-quoted 3-digit padded RAR subject strips surrounding quotes",
			par2Filename:    "",
			subjectFilename: "'movie.part001.rar'",
			headerFilename:  "",
			subjectHeader:   "",
			want:            "movie.part001.rar",
		},
		{
			name:            "clean 3-digit unquoted subject unchanged",
			par2Filename:    "",
			subjectFilename: "movie.part001.rar",
			headerFilename:  "",
			subjectHeader:   "",
		...
		{
			name:            "embedded apostrophe is not surrounding quote and is not stripped",
			par2Filename:    "",
			subjectFilename: "It's.A.Wonderful.Life.part01.rar",
			headerFilename:  "",
			subjectHeader:   "",
			want:            "It's.A.Wonderful.Life.part01.rar",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := selectBestFilename(tt.par2Filename, tt.subjectFilename, tt.headerFilename, tt.subjectHeader)
			if got != tt.want {
				t.Errorf("selectBestFilename(%q, %q, %q, %q) = %q; want %q",
					tt.par2Filename, tt.subjectFilename, tt.headerFilename, tt.subjectHeader, got, tt.want)
			}
		})
	}
}
