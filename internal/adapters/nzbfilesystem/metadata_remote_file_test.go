...
func TestMetadataRemoteFileConfigDefaults(t *testing.T) {
	// Test that configuration defaults work correctly
	config := MetadataRemoteFileConfig{
		GlobalPassword:     "test-password",
		GlobalSalt:         "test-salt", 
		MaxRangeSize:       0, // Use default
	...
	if mrf.streamingChunkSize != DefaultStreamingChunkSize {
		t.Errorf("Expected default streaming chunk size %d, got %d", DefaultStreamingChunkSize, mrf.streamingChunkSize)
	}
	
	// Test custom values
	customConfig := MetadataRemoteFileConfig{
		GlobalPassword:     "test-password",
		GlobalSalt:         "test-salt",
		MaxRangeSize:       16 * 1024 * 1024, // 16MB
	...
	if customMrf.streamingChunkSize != 4*1024*1024 {
		t.Errorf("Expected custom streaming chunk size %d, got %d", 4*1024*1024, customMrf.streamingChunkSize)
	}
	
	t.Logf("Default config: maxRange=%dMB, streamingChunk=%dMB", 
		mrf.maxRangeSize/(1024*1024), mrf.streamingChunkSize/(1024*1024))
	t.Logf("Custom config: maxRange=%dMB, streamingChunk=%dMB", 
		customMrf.maxRangeSize/(1024*1024), customMrf.streamingChunkSize/(1024*1024))
}
