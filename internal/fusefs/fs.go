...
func NewNzbFuseFs(
	...
	metadataService := metadata.NewMetadataService(cfg)
	healthRepository, err := database.NewHealthRepository(cfg)
	if err != nil {
		return nil, fmt.Errorf("failed to create health repository: %w", err)
	}
	poolManager := pool.NewPoolManager(cfg, health.NewHealthChecker(cfg, healthRepository), metadataService)

	metadataRemoteFile := nzbfilesystem.NewMetadataRemoteFile(
		metadataService,
		healthRepository,
		poolManager,
		config.NewConfigGetter(cfg),
	)
