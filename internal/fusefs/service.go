...
func (s *FuseService) Start(ctx context.Context) error {
	...
	fs, err := NewNzbFuseFs(cfg, readaheadBytes)
	if err != nil {
...
func (s *FuseService) Stop(ctx context.Context) error {
	...
	if err := s.server.Unmount(); err != nil {
		// Fallback to fuse.Unmount if server.Unmount fails (e.g. if forced)
		if unmountErr := fuse.Unmount(s.mountpoint); unmountErr != nil {
			return fmt.Errorf("failed to unmount: %v (fallback error: %v)", err, unmountErr)
		}
	}
