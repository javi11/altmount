...
func (s *Server) handleRegenerateSymlinks(c *fiber.Ctx) error {
	...
	for _, file := range files {
		...
		// Create the symlink
		relActualPath, err := filepath.Rel(baseDir, actualPath)
		if err == nil {
			if err := os.Symlink(relActualPath, symlinkPath); err != nil {
				errorCount++
				errors = append(errors, fmt.Sprintf("%s: failed to create relative symlink: %v", file.FilePath, err))
				continue
			}
		} else {
