...
type MetadataVirtualFile struct {
	...
	// randomReadCache is a small per-file LRU of full segment bytes used
...
func (mvf *MetadataVirtualFile) Close() error {
	// Fire cancel on the in-flight reader BEFORE contending for mvf.mu.
	// A concurrent Read can hold mvf.mu for the full segment download
	// latency (15s per-attempt timeout × 5 retries). Cancelling the
	// reader's context first lets that Read unblock and release the lock
	// so Close completes quickly. Safe to call without mvf.mu — uses an
	// atomic handle. See STREAMING_INVARIANTS.md S9.
...
// closeCurrentReader detaches the current reader and hands it to the
// bounded closer pool for asynchronous Close. This avoids blocking
// Seek on UsenetReader.Close() which may wait for in-flight downloads.
// If the pool's queue is full (more pending closes than
// closerWorkerCount can drain) the close runs inline as backpressure
// rather than spawning an unbounded fan-out. See STREAMING_INVARIANTS.md S6.
